...
describe('chain.js (Lumina class)', () => {
  ...
  beforeEach(() => {
    ...
    canvas.getPixelData.mockReturnValue({
  ...
  describe('chainable methods', () => {
    ...
    it('should add brightness operation with argument', () => {
      ...
      expect(lumina.operations[0]).toEqual({
        fn: filters.brightness,
        args: [10],
      });
    });
  ...
  describe('execution methods', () => {
    describe('_resolveSource', () => {
      ...
      it('should throw on unsupported source type', async () => {
        lumina.source = 123;
        await expect(lumina._resolveSource()).rejects.toThrow(
    ...
    describe('toCanvas', () => {
      it('should render and put pixel data on provided canvas', async () => {
        ...
        expect(canvas.putPixelData).toHaveBeenCalledWith(
          mockTargetCanvas,
          mockImageData,
        );
    ...
    describe('toBlob', () => {
      it('should render and return canvas blob', async () => {
        ...
        expect(canvas.canvasToBlob).toHaveBeenCalledWith(
          expect.anything(),
          'image/jpeg',
          0.8,
        );
    ...
    describe('toHtmlElement', () => {
      it('should throw if element not found by ID', async () => {
        await expect(lumina.toHtmlElement('non-existent')).rejects.toThrow(
      ...
      it('should throw if element is not img or canvas', async () => {
        ...
        await expect(lumina.toHtmlElement(div)).rejects.toThrow(
          /only supports <img> and <canvas>/,
        );
      });
    });
  });
